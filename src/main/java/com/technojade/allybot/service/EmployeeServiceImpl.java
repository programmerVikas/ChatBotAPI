...
public class EmployeeServiceImpl implements EmployeeService {
	...
	@Autowired
	private EmployeeDepartmentMappingService emplMappingService;
...
//					.filter(d -> d.getDepId() == e.getDepartmentId()).findAny();
			...
			List<EmployeeDepartmentMapping> listOfEmpDept = emplMappingService.listOfEmpDept(e.getEmpId());
			List<EmployeeDepartmentMappingDto> empDeptDtoList = new ArrayList<EmployeeDepartmentMappingDto>();
			listOfEmpDept.forEach(l -> {
				EmployeeDepartmentMappingDto departmentMappingDto = new EmployeeDepartmentMappingDto();
				GroupMst groupDetailById = groupService.getGroupDetailById(l.getDeptMappingId());
			...
			employeedDto.setListOfDep(empDeptDtoList);
...
//					.filter(d -> d.getDepId() == e.getDepartmentId()).findAny();
	...
	public Employee findByEmailId(String email) {
		...
		Employee emp = null;
		try {
			emp = employeeRepository.findByEmailAndIsActive(email, true);
		} catch (Exception e) {
			// TODO: handle exception
			log.info("Data inconsistency error....");
		}
		return emp;
	}
